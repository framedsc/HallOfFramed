	...
	App.ImageGrid = function(images, container, rowTargetHeight, borderOffset) {
		...
		this.initialize = function(images, container, rowTargetHeight, borderOffset) {
			...
			for(var i = 0; i < this.originalImages.length; i++) {
				var width = parseInt(this.originalImages[i].thumbWidth);
				var height = parseInt(this.originalImages[i].thumbHeight);
				width = width * (this.targetHeight / height); 

				var image = {
					'id': this.originalImages[i].id,
					'author': this.originalImages[i].author,
					'authorsAvatarUrl': this.originalImages[i].authorsAvatarUrl,
					'data': this.originalImages[i].date,
					'gameName': this.originalImages[i].gameName,
					'score': this.originalImages[i].score,
					'fullWidth': this.originalImages[i].width,
					'fullHeight': this.originalImages[i].height,
					'shotUrl': this.originalImages[i].shotUrl,
					'thumbnailUrl': this.originalImages[i].thumbnailUrl,
					'width': width,
		...
		this.renderGrid = function(rows) {
			...
			for(var i = 0; i < rows.length; i++) {
				...
				for(var n = 0; n < rows[i].length; n++) {
					var image = rows[i][n];
					output += '<img src="' + image.thumbnailUrl + '" style="width:' + Math.ceil(image.width) + 'px; height:' + image.height + 'px;" loading=lazy/>';
